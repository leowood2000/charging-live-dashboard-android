...
最后更新：2026-08-10，目标发布版本：v0.11.11。
...
1. `wireless_buck_input effective`：上游无线平台输入 ICL，是首页“无线输入 ICL”的来源。
...
- 最终无线输入 ICL 仍以 `wireless_buck_input effective` 为准。
...
- 活跃无线充电优先读实时 `mca_platform_cp/ibus_total`：`≤20mA` 为 Buck，`≥100mA` 为 CP，`>20mA 且 <100mA` 为切换态。实机已确认 CP 预启动会短暂出现 3–5mA，真正承载后跃升到 400mA 以上，禁止恢复“任意非零即 CP”的判断。
- 无线输入仍连接且 `ibus_total` 可读时始终采用实时判断，包括 `chg_enable=0`、电池电流已降到 0 的自动停充阶段；禁止此时回退停充前的 `work_mode` / `operation mode`。只有无线未连接或节点缺失时才允许回退当前会话日志。
