...
- 有线会话不再只显示“接入/设置电流/移除”；新增协议变化、CP 模式、CP 分压比、CP 阶段和 Buck 并行启停事件。
- `3000→5000→3000mA` 等真实电流序列完整保留。5000mA 可能是 CP 运行期间的临时 Buck 并行电流，不能被压缩成假象的 `3000→3000`。
- 会话来源“有线/无线”只表示物理输入来源，Buck/CP 路径事件单独记录。

## v0.11.27 有线 CP 摘要与 UI 时序

- 有线 CP 当前电池上限仍按 Quick Charge Final/阶段目标/QC 调节目标、当前 div MONITOR-BAT 与 SIC-BAT 的最小值计算；主卡只展示最小值及最终瓶颈，完整候选保留在详情提示。
- 日志投票通道在刷新边界暂时为空时，同一输入会话保留上一张限制卡；会话或路径改变不复用旧卡，避免时序闪烁与跨会话污染。
...
- `effectiveOnly()` 必须先判断投票主题对象是否存在；首次快速采集时日志投票可能尚未捕获，不能直接读取 `result`。
- 该保护只处理前端渲染时序，不改变仲裁算法；空主题显示待捕获，下一轮日志采集后恢复。
...
- `chg_enable` 只有在当前有线会话边界之后仍存在启用票、且驱动 effective 的日志时间不早于该边界时，才可用于停充后的输入测量源选择；否则按未知处理。
- 无线 Buck 输入 ICL 的当前摘要与详情均只接受有效 `wireless_buck_input` effective；没有 effective 时不得按 MIN 推算，也不得用 `wireless_qc` 残留或执行层 ICL 反推仲裁结果。
- `session_at=0` 表示没有可验证的当前会话边界，共享 `buck_charge_curr` 不得据此作为当前 FCC。
- 有线 CP 温控证据同时检查当前比例的 `div*_single` 与 `div*_multi`；两表异值时显示拓扑待确认。
